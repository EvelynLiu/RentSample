		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					objItem.setPrice(getTagValue("price", eElement));
					objItem.setDesc(getTagValue("desc", eElement));
					objItem.setAddress(getTagValue("address", eElement));
					objItem.setType(getTagValue("type", eElement));
					objItem.setSize(getTagValue("size", eElement));
